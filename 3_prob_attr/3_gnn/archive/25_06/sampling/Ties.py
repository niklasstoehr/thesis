...
class Ties():
    ...
    def ties(self,size,phi):
        V = self.g_complete.nodes()
        ...
        #Loops run till sample size * length of V where V is number of nodes in graph as calculated above.
            edges_sample = random.sample(self.g_complete.edges(), 1)
        ...
        for x in self.G1.nodes():
            neigh = (set(self.G1.nodes()) & set(list(self.g_complete.neighbors(x))))
